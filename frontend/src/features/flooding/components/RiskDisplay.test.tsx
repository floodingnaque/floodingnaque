  ...
  describe("Styling", () => {
    it("should apply green color classes for risk level 0", () => {
      const { container } = render(
        <RiskDisplay riskLevel={0} probability={0.15} />,
      );
      const heading = screen.getByText("Safe");
      expect(heading.className).toContain("text-risk-safe");
      // Background should be risk-safe
      expect(container.firstChild).toHaveClass("bg-risk-safe/15");
    });

    it("should apply amber color classes for risk level 1", () => {
      const { container } = render(
        <RiskDisplay riskLevel={1} probability={0.65} />,
      );
      const heading = screen.getByText("Alert");
      expect(heading.className).toContain("text-risk-alert");
      expect(container.firstChild).toHaveClass("bg-risk-alert/15");
    });

    it("should apply red color classes for risk level 2", () => {
      const { container } = render(
        <RiskDisplay riskLevel={2} probability={0.9} />,
      );
      const heading = screen.getByText("Critical");
      expect(heading.className).toContain("text-risk-critical");
      expect(container.firstChild).toHaveClass("bg-risk-critical/15");
    });
  ...
  describe("All Risk Levels", () => {
    ...
    cases.forEach(({ level, label, prob }) => {
      it(`should render correctly for risk level ${level} (${label})`, () => {
        ...
        // Percentage appears in both the main display and the progress bar label
        expect(
          screen.getAllByText(`${Math.round(prob * 100)}%`).length,
